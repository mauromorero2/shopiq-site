...
export default function WindowChrome({
  ...
  infoLeft?: string; infoCenter?: string; infoRight?: string;
  ...
  return (
    <div className="relative bg-white" style={{ width, height, border: `2px solid ${stroke}` }}>
      <svg viewBox={`0 0 ${width} ${height}`} width={width} height={height} shapeRendering="crispEdges">
        ...
        {!hideTitleLines && Array.from({length: Math.floor((tb-12)/4)}).map((_,i)=>{
          const y=6+i*4; return <line key={i} x1={6} y1={y} x2={width-6} y2={y} stroke={stroke}/>;
        })}
        {/* Bezel centrale + doppia riga */}
        {!!title && (
          <>
        ...
        {/* Corner widgets */}
        <rect x={3} y={height-50} width={18} height={18} fill="#fff" stroke={stroke}/>
        <polygon points={`7,${height-23} 11,${height-29} 11,${height-17}`} fill="none" stroke={stroke}/>
        <rect x={width-20} y={height-50} width={18} height={18} fill="#fff" stroke={stroke}/>
        <polygon points={`${width-6},${height-23} ${width-10},${height-29} ${width-10},${height-17}`} fill="none" stroke={stroke}/>
        <rect x={width-20} y={tb+ib+2} width={18} height={18} fill="#fff" stroke={stroke}/>
        <polygon points={`${width-11},${tb+ib+6} ${width-17},${tb+ib+10} ${width-5},${tb+ib+10}`} fill="none" stroke={stroke}/>
      </svg>

      {/* Titolo (Jersey 10) */}
      {!!title && (
        <div className="absolute text-[18px] leading-none"
      ...
      <div className="absolute top-[40px] flex w-full items-center justify-between px-3 text-[16px] leading-none">
        <span>{infoLeft}</span><span>{infoCenter}</span><span>{infoRight}</span>
      </div>
      {/* Contenuto */}
      <div className="absolute" style={{left:1, top: tb+28, width: width-2, height: height-(tb+28)-2, overflow:"hidden"}}>
        {children}
      </div>
      {/* Handle invisibile solo sulla titlebar (drag realistico) */}
      <div data-drag-handle="1" className="absolute left-0 top-0 h-[40px] w-full" />
    </div>
  );
}
